...
M.convert = function(filepath, prev_exported_name, new_exported_name)
   ...
   for _, line in ipairs(lines) do
      ...
      if stripped:match("export%s+default%s+async%s+function%s+" .. prev_exported_name) then
         local replaced = line:gsub(
            "export%s+default%s+async%s+function%s+" .. prev_exported_name,
      ...
      elseif stripped:match("export%s+default%s+function%s+" .. prev_exported_name) then
         local replaced = line:gsub(
            "export%s+default%s+function%s+" .. prev_exported_name,
      ...
      elseif stripped:match("export%s+default%s+" .. prev_exported_name .. "%s*;") then
